// ============= GLOBAL VARIABLES =============
let activeCharts = {};
let notifiedBudgets = new Set(); // Track budgets đã hiển thị thông báo
// ============= GLOBAL FUNCTIONS =============
...
// --- CHUẨN HÓA VÀ LƯU TRẠNG THÔNG BÁO (1 session) ---
// Khởi tạo Set lưu ID dưới dạng string để so sánh luôn nhất quán
if (!window.notifiedBudgets) {
    const saved = JSON.parse(sessionStorage.getItem('notifiedBudgets') || '[]');
    // đảm bảo mọi phần tử đều là string
    window.notifiedBudgets = new Set((saved || []).map(x => String(x)));
}

// helper: kiểm tra đã thông báo chưa (chuỗi)
function isNotified(budgetId) {
    return window.notifiedBudgets.has(String(budgetId));
}

// helper: đánh dấu đã thông báo và cập nhật sessionStorage
function markNotified(budgetId) {
    window.notifiedBudgets.add(String(budgetId));
    sessionStorage.setItem('notifiedBudgets', JSON.stringify([...window.notifiedBudgets]));
}

// helper: bỏ đánh dấu (ví dụ khi xóa budget)
function unmarkNotified(budgetId) {
    window.notifiedBudgets.delete(String(budgetId));
    sessionStorage.setItem('notifiedBudgets', JSON.stringify([...window.notifiedBudgets]));
}

// --- HÀM HIỂN THỊ CẢNH BÁO (DÙNG helper trên) ---
function showBudgetWarning(budgetId, percentage, categoryName, spentAmount, budgetAmount) {
    if (isNotified(budgetId)) return; // đã thông báo rồi -> thoát

    // chỉ đánh dấu khi chắc sẽ hiển thị cảnh báo (tránh đánh dấu nếu không có cảnh báo)
    let title, text, icon, color;
    if (percentage >= 100) {
        title = '⚠️ Vượt Ngân Sách!';
        text = `Ngân sách "${categoryName}" đã vượt mức!<br>
                Đã chi: ${formatCurrencyVND(spentAmount)}<br>
                Ngân sách: ${formatCurrencyVND(budgetAmount)}<br>
                Vượt: ${formatCurrencyVND(spentAmount - budgetAmount)}`;
        icon = 'error';
        color = '#dc3545';
    } else if (percentage >= 90) {
        title = '🚨 Gần Hết Ngân Sách!';
        text = `Ngân sách "${categoryName}" đã sử dụng ${percentage}%!<br>
                Còn lại: ${formatCurrencyVND(budgetAmount - spentAmount)}`;
        icon = 'warning';
        color = '#dc3545';
    ...
    // đánh dấu trước (đảm bảo dù người dùng tắt popup vẫn không hiện lại)
    markNotified(budgetId);
    ...
    document.addEventListener("DOMContentLoaded", async function () {
        ...
        if (form) {
            form.addEventListener("submit", async (e) => {
                ...
                // Nếu là edit mode, thêm BudgetID và UserID
                if (isEditMode) {
                    budgetData.budgetID = parseInt(editId, 10);
                    budgetData.userID = parseInt(userId, 10);
                } else {
                    budgetData.userID = parseInt(userId, 10);
                }
                ...
                try {
                    ...
                    budgetData.userID = String(budgetData.userID);
